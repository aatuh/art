...
mod tests {
    ...
    use crate::planet::{CelestialFrame, Vec3d};
    ...
    #[test]
    fn j2000_noon_places_the_subsolar_point_near_greenwich() {
        let frame = CelestialFrame::at_seconds_since_j2000(0.0);
        let fixed_sun = inertial_to_earth_fixed(frame.sun.center_m, frame.earth_rotation_radians);
        let subsolar_longitude_degrees = fixed_sun.z.atan2(fixed_sun.x).to_degrees();

        assert!(subsolar_longitude_degrees.abs() < 2.0);
    }
